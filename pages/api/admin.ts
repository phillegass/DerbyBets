...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (action === 'open') { state.status = 'open'; delete state.winnerHorseId; }
    else if (action === 'close') { state.status = 'closed'; }
    else if (action === 'setWinner') {
      if (!winnerHorseId) return res.status(400).json({ error: 'No winner provided' });
      state.status = 'finished'; state.winnerHorseId = Number(winnerHorseId);
